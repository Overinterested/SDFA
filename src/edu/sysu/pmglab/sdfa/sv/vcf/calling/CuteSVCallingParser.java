...
public class CuteSVCallingParser extends AbstractCallingParser {
    ...
    public void parse(int indexOfFile, Chromosome chromosome, int pos, ByteCodeArray encodeNoneFieldArray, ReusableMap<ByteCode, ByteCode> infoField, SVGenotypes svGenotypes) {
            ...
            if (svTypeSign1.getIndex() == BND_TYPE_INDEX) {
                if (chr2ByteCode == null) {
                    ...
                    if (bndInfo != null) {
                        chromosome2 = contig.get(bndInfo.getKey());
                        pos2 = wrapToInt(bndInfo.getValue());
                        loadTwoLatest(indexOfFile, chromosome, chromosome2, svTypeSign1, svTypeSign1, pos, pos2,
                                end1, end2, len1, len2, encodeNoneFieldArray, infoField, svGenotypes);
                        return;
                    }
                } else {
                    chromosome2 = wrapToChr(chr2ByteCode, chromosome);
                    if (end1 == -1 && pos2 == -1) {
                        ...
                        pos2 = pos2 == -1 ? end1 : pos2;
                        loadTwoLatest(indexOfFile, chromosome, chromosome2, svTypeSign1, svTypeSign1, pos, pos2,
                                -1, -1, -1, -1, encodeNoneFieldArray, infoField, svGenotypes);
                        return;
                    }
                }
            }
        ...
        loadTwoLatest(indexOfFile, chromosome, chromosome, svTypeSign1, svTypeSign2, pos, pos2,
                end1, end2, len1, len2, encodeNoneFieldArray, infoField, svGenotypes);
    }
}
